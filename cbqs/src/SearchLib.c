...
int ctg(solver_ctx_t *ctx, model_t *mod, state_t *cur_sol, callback_t callback, incumbents_t *incumbents) {
	...
	while (mod->M > 0 && total_oracles < (size_t) mod->M
		...
		if (res) {
			 ...
			 * bd xjs: never publish a FEASIBLE state whose tot_profit is not an
			 * objective. copy_state_inplace copies tot_profit AND the feasibility
			 * flag, and solve() reports both (objective / feasible / solution), so
			 * a stage-2 slack sum landing here with the now-truthful feasible==1
			 * would claim "feasible, objective = <slack>" -- a loudly-wrong answer
			 * turned silently wrong (§2.1). The clause is deliberately NOT the
			 * stronger `profit_is_objective &&`: an INfeasible state's tot_profit
			 * is already documented as a violation sum rather than an objective
			 * (result.feasible == False), and blocking those would change which
			 * point a never-feasible run reports -- measured: the best-violation
			 * incumbent becomes the untouched greedy start, for no correctness
			 * gain (global_opt is seeded with a violation sum by
			 * initial_state_preparation either way). So the narrow clause is a
			 * strict no-op everywhere except the stage-2 case this fix creates.
			 * SATISFY keeps profit_is_objective == 1 throughout, so it is exempt. */
			cbqs_mutex_lock(&update_lock);
			if ((profit_is_objective || !cur_sol->feasible)
			    && mod->global_opt->tot_profit > cur_sol->tot_profit){
