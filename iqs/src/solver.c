...
//    else sw_clrbit(cur_sol->vector, index); // set assignment to 0 (just to make sure, it should already be 0)
    ...
    if (next_assignment) bool_ = evaluation(con, potentials, index, positive_indices, num_positive_indices, positive_offsets, cur_sol, POSITIVE);
    else bool_ = evaluation(con, potentials, index, negative_indices, num_negative_indices, negative_offsets, cur_sol, NEGATIVE);

    if (bool_){
        if (index == depth) (*count_solutions)++;
        else{
            if (next_assignment) {
                update_potentials(con, potentials, index, positive_indices, num_positive_indices, positive_offsets, cur_sol, next_assignment, POSITIVE, PLAIN);
                sw_setbit(cur_sol->vector, index); // set assignment to 1
            }
            else {
                update_potentials(con, potentials, index, negative_indices, num_negative_indices, negative_offsets, cur_sol, next_assignment, NEGATIVE, PLAIN);
...
//            if (!b1 && !b2) (*count_solutions)++;
            ...
            if (next_assignment) update_potentials(con, potentials, index, positive_indices, num_positive_indices, positive_offsets, cur_sol, next_assignment, POSITIVE, INVERSE);
            else update_potentials(con, potentials, index, negative_indices, num_negative_indices, negative_offsets, cur_sol, next_assignment, NEGATIVE, INVERSE);
        }
    }
...
int initial_state_preparation(state_t *new_sol, state_t *cur_sol, int n, int NTerms,
